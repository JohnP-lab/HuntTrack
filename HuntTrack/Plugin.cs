...
public sealed class Plugin : IDalamudPlugin
{
    ...
    public Plugin()
    {
        ...
        string path = Path.Combine(PluginInterface.AssemblyLocation.Directory?.FullName!, @"Data", fileName);
        using FileStream json = File.OpenRead(path);
        this.cibles = JsonSerializer.Deserialize<List<Cible>>(json, _options);
        */
    }
    ...
    public void LoadJson()
    {
        try
        {
            ...
            if (cleanJson.Version != saveJson.Version)
            {
                ...
                {
                    var oldCible = saveJson.Cibles.FirstOrDefault(c => c.ModelId == newCible.ModelId);
                    if (oldCible != null)
                    {
